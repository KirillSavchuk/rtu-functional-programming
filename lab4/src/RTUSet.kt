...
val Set.contents: String get() = "{" + iterateSet(this, -BOUND).substring(1) + "}"

fun iterateSet(set: Set, x: Int): String {
    return when {
        x > BOUND -> ""
        set.contains(x) -> "," + x.toString() + iterateSet(set, x + 1)
        else -> iterateSet(set, x + 1)
    }
}
